        ...
        function Addplayers(cache,identity) {
            ...
            for (const i in cache.players) {
              if (!cachefuck[cache.players[i].id]) {
                ...
                // }
                ping = '<i class="fa fa-wifi"></i> '+cache.players[i].ping+''
                if (cache.players[i].ping > 300) {
                  ping = '<i style="color:red;" class="fa fa-wifi"></i> '+cache.players[i].ping+''
                } else if (cache.players[i].ping > 100) {
                  ping = '<i style="color:yellow;" class="fa fa-wifi"></i> '+cache.players[i].ping+''
                }
                var divid = `<span class="leaderboard__name" style="float: left;position: absolute;left: -1.5vw;bottom: 0%;font-weight: 700;font-size: 14px;width: auto;min-width: 55px;background: #0808087d;border-radius: 5px;color: #daebf5;"><i class="fa fa-id-card"></i> `+cache.players[i].id+`</span>`
                if (!showid && !admin) {
                  divid = ''
                }
                var admindiv = ''
                if (cache.players[i].admin) {
                  admindiv = `<span class="leaderboard__name admin" style="float: left;position: absolute;left: 3vw;font-weight: 700;font-size: 15px;bottom: 8%;color: gold;"><i class="fa fa-crown"></i></span>`
                }
                if (!showadmins) {
                  admindiv = ''
                }
                ...
                if (cache.players[i].vip) {
                  vipdiv = `<span class="leaderboard__name vip" style="float: left;position: absolute;left: 30px;font-weight: 700;font-size: 15px;bottom: 22px;color: #d206d2;"><i class="fa fa-star"></i></span>`
                }
                if (!showvip) {
                  vipdiv = ''
                }
                var jobdiv = `<span class="leaderboard__name job" style="float: left;position: absolute;left: 30vw;font-weight: 700;font-size: 15px;"><i class="fa fa-user-tie"></i> `+capitalizeFirstLetter(cache.players[i].job)+`</span>`
                if (!showjobs && !admin) {
                  jobdiv = ''
                }
                var discdiv = ''
